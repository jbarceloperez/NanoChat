...
import es.um.redes.nanoChat.server.roomManager.Manager;
...
public class NanoChatServer implements Runnable {
	...
	private static final int MYPROTOCOL = 73608090;
    ...
    private NanoChatServer(InetSocketAddress a)
    {
    	...
    	Manager sala = new Manager("B");
    	manager.registerRoomManager(sala);
	...
	public void run()
	{
   		try {
   			...
   			while (true)	//siempre escuchando y siempre va creando hilos para la gente que se quiera conectar!
   			{
   				// Espera nuevas conexiones
   				// Las accepta y obtiene el socket específico de cada conexión
   				Socket s = serverSocket.accept(); //espera una conexion
   				...
   				new NCServerThread(manager,s).start();	//crear un nuevo hilo que se le pasa el manager de este servidor y el socket
   			}
   		} catch (IOException e) {
   			e.printStackTrace();
   		}
    ...
    public void init()
    {
        try {
            ...
            serverSocket = new ServerSocket(); //crea el server socket
    	...
    	new Thread(this).start(); //se ejecuta el metodo run
