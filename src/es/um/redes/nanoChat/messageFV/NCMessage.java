...
public abstract class NCMessage {
	...
	public static final byte OP_SALAS = 10;
	
	public static final byte OP_ENTRAR = 20;
	public static final byte OP_STATS = 30;
	...
	/**
	 * Códigos de los opcodes válidos  El orden				AQUÍ ES DONDE VAN LOS CÓDIGOS VALIDOS Y TENDREMOS QUE IR AÑADIENDO
	...
	public static NCMessage readMessageFromSocket(DataInputStream dis) throws IOException {
		...
		if (!lines[0].isEmpty()) { // Si la línea no está vacía
			...
			if (code == OP_INVALID_CODE)
				return null;
			switch (code) {
			case OP_NICK:	//UNA VEZ QUE SE RECIBE UN MENSAJE DE TIPO NICK
			{
				return NCRoomMessage.readFromString(code, message);
			}
			case OP_STATS:	//UNA VEZ QUE SE RECIBE UN MENSAJE DE TIPO STATS
			{
				//return NCStats.readFromString(code, message);	//EL MESSAGE SERÁ UN MENSAJE EN STRING QU ECODIFICARÁ TODO LO NECESARIO EN ESE NCStats
			}
			default:
	...
	//public static NCMessage makeStatsMessage(byte code, List<String> users, List<Integer> nmessages) {
		//return new NCStats(code, users, nmessages);
	}
//}
